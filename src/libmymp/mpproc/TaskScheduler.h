...
public:
    ...
    /**
     * @brief 在CPU 对齐流水线中读取 `GetInputList` 对应的数据。
     * @par 参数
     * 无。
    ...
    /**
     * @brief 在CPU 对齐流水线中读取 `GetDNamelist` 对应的数据。
     * @par 参数
     * 无。
     * @return 返回该步骤计算、查询或状态判断的结果。
     */
    ...
    /**
     * @brief 在CPU 对齐流水线中读取 `GetOutput` 对应的数据。
     * @par 参数
     * 无。
     * @return 返回该步骤计算、查询或状态判断的结果。
     */
...
protected:
    /**
     * @brief 在CPU 对齐流水线中处理 `CreateReader` 对应的数据。
    ...
    /**
     * @brief 在CPU 对齐流水线中处理 `CreateQrsReader` 对应的数据。
     * @param maxstrlen 控制当前步骤范围或规模的 `maxstrlen`。
     * @param mapped 供该函数读取或更新的 `mapped` 参数。
     * @param ndatbufs 控制当前步骤范围或规模的 `ndatbufs`。
     * @param nagents 控制当前步骤范围或规模的 `nagents`。
    ...
    /**
     * @brief 在CPU 对齐流水线中读取 `GetDataFromReader` 对应的数据。
    ...
    /**
     * @brief 在CPU 对齐流水线中读取 `GetReferenceData` 对应的数据。
    ...
    /**
     * @brief 在CPU 对齐流水线中处理 `CreateAlnWriter` 对应的数据。
    ...
    /**
     * @brief 在CPU 对齐流水线中通知 `NotifyAlnWriter` 对应的数据。
     * @par 参数
     * 无。
     * @return 无返回值；结果写入传入缓冲区、输出参数或对象状态。
     */
    ...
    /**
     * @brief 在CPU 对齐流水线中等待 `WaitForAlnWriterToFinish` 对应的数据。
    ...
    /**
     * @brief 在CPU 对齐流水线中处理 `CreateMemoryConfigs` 对应的数据。
    ...
    /**
     * @brief 在CPU 对齐流水线中处理 `ProcessPart` 对应的数据。
     * @param MpBatch 供该函数读取或更新的 `MpBatch` 参数。
     * @param workerid 供该函数读取或更新的 `workerid` 参数。
    ...
    /**
     * @brief 在CPU 对齐流水线中读取 `GetAlnWriter` 对应的数据。
     * @par 参数
     * 无。
     * @return 返回该步骤计算、查询或状态判断的结果。
     */
    ...
    /**
     * @brief 在CPU 对齐流水线中读取 `GetQrsReader` 对应的数据。
     * @par 参数
     * 无。
     * @return 返回该步骤计算、查询或状态判断的结果。
     */
    ...
    /**
     * @brief 在CPU 对齐流水线中设置 `SetChunkDataAttributesReaders` 对应的数据。
