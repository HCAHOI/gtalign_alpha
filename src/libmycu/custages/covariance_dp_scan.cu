...
/**
 * @brief 在CUDA 刚体拟合与评分中复制 `CopyCCDataToWrkMem2_DPscan` 对应的数据。
...
// 
/**
 * @brief 在CUDA 刚体拟合与评分中计算 `CalcCCMatrices64_DPscan` 对应的数据。
...
/**
 * @brief 在CUDA 刚体拟合与评分中搜索 `FindD02ThresholdsCCM_DPscan` 对应的数据。
 * @param ndbCstrs 当前批次中的参考结构数量。
 * @param ndbCposs 当前批次中参考结构的总位置数。
 * @param maxnsteps 每对结构保留的候选搜索步数。
...
template<int READCNST>
/**
 * @brief 在CUDA 刚体拟合与评分中计算 `CalcCCMatrices64_DPscanExtended` 对应的数据。
 * @param nqystrs 当前批次中的查询结构数量。
 * @param ndbCstrs 当前批次中的参考结构数量。
 * @param ndbCposs 当前批次中参考结构的总位置数。
 * @param dbxpad 参考数据行末用于对齐访问的填充长度。
...
/**
 * @brief 在CUDA 刚体拟合与评分中计算 `CalcScoresUnrl_DPscan` 对应的数据。
 * @param nqystrs 当前批次中的查询结构数量。
 * @param ndbCstrs 当前批次中的参考结构数量。
 * @param ndbCposs 当前批次中参考结构的总位置数。
 * @param dbxpad 参考数据行末用于对齐访问的填充长度。
